...
public class LampiS3Transfer {
    ...
    private Supplier<S3ObjectInputStream> constructSupplier(String downloadFilename) {
        return () -> ovaraS3Client.getObject(config.ovaraS3Bucket(), downloadFilename).getObjectContent();
    }

    private PartETag submitTaskForUploading(String uploadFilename, ByteArrayInputStream inputStream, boolean isFinalPart) {
        ...
        System.out.printf(String.format("Submitting uploadPartId: %d of partSize: %d%n", eachPartId, inputStream.available()));
    ...
    public String transferToLampi(String filename, int numberOfFiles) throws Exception {
        ...
        InitiateMultipartUploadRequest initRequest = new InitiateMultipartUploadRequest(config.lampiS3Bucket(), filename)
                .withObjectMetadata(metadata);
        InitiateMultipartUploadResult initResult = lampiS3Client.initiateMultipartUpload(initRequest);

        uploadId = initResult.getUploadId();

        List<Supplier<S3ObjectInputStream>> streamsFList = IntStream.rangeClosed(1, numberOfFiles).mapToObj(fileNumber -> {
            String downloadFilename = fileNumber == 1 ? filename : String.format("%s_part%s", filename, fileNumber);
            return constructSupplier(downloadFilename);
        }).toList();

        Enumeration<Supplier<S3ObjectInputStream>> streams = Iterators.asEnumeration(streamsFList.iterator());
        ...
        while ((bytesRead = inputStream.read(data, 0, data.length)) != -1) {
            ...
            PartETag partETag = submitTaskForUploading(filename, new ByteArrayInputStream(bufferOutputStream.toByteArray()), false);
            parts.add(partETag);
            bufferOutputStream.reset(); // flush the bufferOutputStream
            bytesAdded = 0; // reset the bytes added to 0
        }

        PartETag partETag = submitTaskForUploading(filename, new ByteArrayInputStream(bufferOutputStream.toByteArray()), true);
        parts.add(partETag);

        CompleteMultipartUploadRequest completeRequest = new CompleteMultipartUploadRequest(
                config.lampiS3Bucket(),
                filename,
                uploadId,
                parts);
        CompleteMultipartUploadResult completeMultipartUploadResult = lampiS3Client.completeMultipartUpload(completeRequest);
